...
62 cuts・58 wellsで完了し、昇格しなかった。prefix平面の終点U変化相関は
`0.923--0.927`、固定grid oracleは`8.9038 -> 8.2644`（`-0.6394`）だったが、
固定primaryは`10.0676`（`+1.1638`）へ悪化した。bootstrap区間は
`[+0.6004,+1.6091]`、standard `0/5`、branch `0/5`、fraction `0/4`。

物理的な傾斜は観測可能だが、強いbaseの残差補正方向ではない。prefix-only XY planeの
追加は棄却し、予約120 wellsは未使用のままStage 28へ進む。
