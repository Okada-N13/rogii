...
現在のactive taskは **Stage 25A: temporal path decoder screen** である。

1. T4/L4で`notebooks/620_run_stage25a_temporal_path_decoder.ipynb`を実行する。
2. Stage 24A checkpointsを固定しTCNを再学習しない。
3. 62 design-validation cutsでsmooth mean、median、Viterbiを事前固定比較する。
4. これはexploratory screenで、最良profileをそのまま採用しない。
5. 強い全gate通過時だけ500-cut OOF nested auditへ進む。
6. 予約120 wellsは使わず、Kaggle submissionも作らない。
