...
現在のactive taskは **Stage 21A: visible-prefix candidate router** である。

1. Colab CPUで`notebooks/540_run_stage21a_prefix_router.ipynb`を単独実行する。
2. Stage 20A/Bの計297 discovery wellsを完全除外する。
3. outer prefix内の2 internal cutsで10候補を実測backtestする。
4. self-selected候補をtop-PF A130へ固定25%・12 ft上限で混ぜる。
5. rank correlation、bootstrap、全fold family、P90 gateを確認する。
6. 全gate通過時だけ別well・高PF解像度のStage 21Bへ進む。

Stage 21Aから直接learned routerやKaggle submissionを作らない。
