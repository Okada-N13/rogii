...
# 2026-07-24 update: Stage 33A -> Stage 34A

- Stage 33Aは`-0.1153`、standard/spatial/branch/fraction通過まで改善したが、
  bootstrap上限`+0.00850`、P90 `+0.1394`、typewell `3/5`で不合格。
- continuous gateは予測平均`0.6513`対oracle`0.5741`で補正過多。予約120 wellsは未使用。
- Stage 34Aはtraining OOF scoreだけで保守的hard-gate閾値を固定するCPU監査。
- validation真値による閾値調整、再推論、再学習は行わない。
- Stage 34A不合格ならTCN residual familyを終了する。

# 2026-07-24 update: Stage 34A -> Stage 35A

- Stage 34A hard gateは`-0.1304`、standard `5/5`、P90非悪化まで改善したが、
  bootstrap上限`+0.0151`、fraction/typewell不整合で不合格。
- 予約120 wellsは未使用。rowwise TCN residual familyを終了する。
- Stage 35Aはtraining残差曲線を96点へ正規化し、8 FPCA基底の係数をcut-level HGBで予測する。
- 補正alphaはtraining OOFのcut均等MSEから解析的に固定し、validationでは調整しない。
- 62 design-validation全gate通過時だけStage 35Bで予約120 wellsを一度開封する。

# 2026-07-24 update: Stage 35A -> Stage 36

- Stage 35Aはtraining OOF `-0.2076`だったが、独立検証は`-0.0924`、
  bootstrap上限`+0.2147`、P90`+0.3720`、standard `2/5`で棄却。
- 予約120 wellsは未使用。cut-level residual curve familyを終了する。
- Stage 18C/18Dのlearned donor retrievalは全cut検証で強いが、Kaggleでは時間超過して
  LBスコアを取得できていないため、精度を変えずruntimeだけをStage 36で修正する。
- package v4はtest wellを4並列処理し、1 worker版とのCSV byte完全一致を必須とする。
- hidden 200 wellsの保守的推定が600秒以内の場合だけKaggle Datasetへ昇格する。
- 詳細は`docs/strategy/stage36_parallel_retrieval_runtime.md`。

# 2026-07-24 update: Stage 36 -> Stage 37A

- Stage 36は1 workerと4 workersのCSV byte完全一致を確認し、public 3 wellsで
  `4.81秒 -> 1.09秒`、hidden 200 wellsの保守推定`81.24秒`で全gate通過。
- Stage 18 v4を6.589 top-PF baseへ追加したKaggle submissionは実行中。
- 提出待ちの間に、5点台向け独自モデルの共通評価基盤Stage 37を開始する。
- Stage 37Aは既存500 training wells、62 design cuts、未開封120 confirmation wellsを
  再抽選せず固定し、全short-prefix training cutsへ拡張する。
- Stage 37Aではconfirmation TVTや予測を読まず、manifest hash確定後にStage 37Bへ進む。
- 詳細は`docs/strategy/stage37_pseudo_private_benchmark.md`。

# 2026-07-24 update: Stage 37A -> Stage 37B

- Stage 37Aは全gate通過。固定manifest hashは
  `a51c2776a63515e27aa1177ede37abebfd021dabea5fb93436d2635a97a4d166`。
- trainingは1,370 cuts / 500 wells / 6,997,065 rows、designは62 cuts / 58 wells、
  confirmationは未開封120 wells。全role間well overlapは0。
- Stage 37Bはtraining/designだけへtop-PF相当baseをmaterializeする。
- 20 cuts単位の再開可能Parquetとし、confirmation targetは引き続き読まない。
- Stage 37B完了後、同じ固定base上でStage 38 learned donor retrieval v2を学習する。
