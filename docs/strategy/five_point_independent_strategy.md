...
Stage 12A confirmed substantial oracle headroom and stable Top-10 rank signal, while also showing that raw row-wise argmin is unusable. Stage 12B therefore implements a candidate-shared TCN over all 61 states. It combines four fixed NCC scales with paired horizontal/candidate-typewell GR channels, offset and trajectory context, cross entropy, and an NCC-derived hard-negative margin. Five ordinary OOF models decide whether separate spatial/typewell refits and deterministic K-best decoding are justified. This stage remains validation-only and uses no public model or Kaggle submission.

Stage 12B passed decisively: Top-10 recall reached 82.2%, median true-state rank fell to 3, NLL improved by 2.16, and expected-offset RMSE improved by 1.693 across all five folds. Stage 12C now performs the missing spatial/typewell refits and nested path-profile selection. Its posterior mean and exact Top-16 path mixture are evaluated separately so that a strong emission model is not weakened merely to force a Viterbi-style path component into the pipeline.

Stage 12C rejected all path profiles. Even the 25% soft path blend slightly worsened all three fold families, and stronger continuity monotonically worsened RMSE/P90 despite reducing worst-tail concentration. The independent track therefore retains Stage 12B expected offset and moves to Stage 13 target-free uncertainty shrinkage rather than spending further validation budget on lattice stiffness.

Stage 13 found a stable standard-OOF diversity gain from a 90/5/5 standard/spatial/typewell blend, but every hand-written magnitude/roughness uncertainty rule failed strict holdouts. Stage 14 replaces threshold rules with fold-local generic residual HGBs and a standard-only disagreement/entropy stack, while retaining conservative nested correction caps and the 90/5/5 control.

Stage 14 produced large, consistent residual gains across all three holdout families. Its formal failure came only from relative worst-tail concentration even though absolute worst-tail SSE fell. Stage 14B corrects this metric, extends the boundary-optimal weight/cap grid without retraining, and requires a common absolute-tail-safe generic profile before inference export.
