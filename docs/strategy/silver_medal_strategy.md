...
初回の軽量実装では、GRの9/33/129点系列統計、PF surface・uncertainty、typewell mismatch、trajectory、既知prefixだけにfitした100/500/1000 ft surface trendを入力するHistGradientBoosting residual modelをwell単位でcross-fitした。2学習seed平均・補正強度0.60で全773 wellsのOOFはStage 2の12.565438から12.299725へ改善し、全5 foldsで一貫した。well単位paired bootstrapの95%区間は[-0.324, -0.103]、nested correction weight選択も0.55〜0.70に収まった。一方でwell中央値と最大誤差は悪化したため、Stage 4ではtail保護とdecorrelated path成分を優先する。
