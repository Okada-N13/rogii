...
初回の軽量実装では、GRの9/33/129点系列統計、PF surface・uncertainty、typewell mismatch、trajectory、既知prefixだけにfitした100/500/1000 ft surface trendを入力するHistGradientBoosting residual modelをwell単位でcross-fitした。2学習seed平均・補正強度0.60で全773 wellsのローカルOOFはStage 2の12.565438から12.299725へ改善し、native Colab再現値は12.339250だった。いずれも全5 foldsで一貫して改善した。well単位paired bootstrapの95%区間は[-0.324, -0.103]、nested correction weight選択も0.55〜0.70に収まった。一方でwell中央値と最大誤差は悪化したため、Stage 4ではtail保護とdecorrelated path成分を優先する。
...
初回実装では、PF seed不確実性が上位20%のwellだけStage 3補正を±1 ftに制限し、既知prefixでGR gain/offsetを校正した決定論的Viterbi trellisを10%追加した。ローカルOOFはStage 3の12.299725から12.204505へ改善し、全5 folds、median、P90、maximum well errorが同時に改善した。well単位bootstrapの95%区間は[-0.149, -0.035]だった。worst 5% SSE shareは微改善したがworst 10% shareはわずかに増えたため、tail問題が解消したとは扱わない。trellisの全体最適重み0.175〜0.20ではなく、tail指標とのPareto点である0.10を採用する。
...
初回の4-neighbour spatial residual補正は通常OOFで12.204505→12.131975、全5 foldsとbootstrapで改善し、shuffle controlでは12.236989へ悪化した。しかし6-region spatial-blockでは12.195102と改善が0.0094に縮小し、2 blocksで悪化、worst 10% SSE shareも増加した。探索した60設定のどれも通常・spatial-block双方で0.02以上改善しなかったため、事前gateに従い不採用とし、Stage 4をpromoted modelとして維持する。
...
提出生成は実装済みで、主提出はStage 4、補助controlはStage 3とする。Stage 5空間補正はgate不通過のためtest推論へ含めない。提出CLIはtest推論中にtrainの同一well正解を開かず、contact overrideも使わない。公式sampleとのID集合・順序、重複、欠損、有限値を検証し、ローカルsnapshotでは3 wells・14,151行を正常生成した。Stage 3/4 test予測の相関は約0.999998と高いため、2本を十分にdecorrelatedな提出とは扱わない。
