"""Build the standalone Stage 20B disjoint-well confirmation notebook."""
...
OUTPUT = Path("notebooks/530_run_stage20b_disjoint_confirmation.ipynb")
...
def build() -> None:
    cells = [
        markdown(
            ...
            "Stage 20Aの診断で最も安定していたweight 0.05、cap 8 ftを固定し、"
            "Stage 20Aで使った158 wellsを完全除外した別sampleで確認します。"
            "このNotebookの結果を見てweightを変更せず、全gate通過時だけStage 20Cへ進みます。"
            "Kaggle提出は作りません。CPUランタイムを使用してください。\n"
        ),
        ...
        markdown(
            "## 固定artifactとdiscovery exclusion\n\nStage 20Aの`cut_features.parquet`から"
            "discovery wellsを読み、候補poolから先に除外します。cut単位ではなくwell単位の完全除外です。\n"
        ),
        code(
        ...
        markdown(
            "## 独立well confirmation\n\nA130/SP45 proxyの計算量はStage 20Aと同程度です。"
            "25 cutsごとに進捗を表示します。\n"
        ),
        code(
            ...
            "'--config','configs/experiment/stage20b_disjoint_confirmation.yaml',"
            "'--stage16b-run',str(stage16b_run),'--stage17a-run',str(stage17a_run),"
            "'--exclude-run',str(stage20a_run),'--data-dir',str(data_dir),"
            "'--artifact-dir',str(artifact_dir),'--run-id',RUN_ID])\n"
            "summary=json.loads((run_dir/'summary.json').read_text())\n"
            "{\n"
            " 'stage20b_complete':summary['stage20b_complete'],"
            " 'promoted_to_stage20c':summary['promoted_to_stage20c'],"
        ...
        markdown(
            "最後の辞書とweight表を共有してください。昇格判定はweight 0.05だけで完結しており、"
            "診断表の別weightへ変更してStage 20Bを再実行しません。\n"
        ),
    ]
    notebook = {
        "cells": cells,
        "metadata": {
            ...
            "stage20b": {
                "submission": False, "standalone_setup": True,
                "discovery_well_exclusion": True, "fixed_weight": 0.05, "fixed_cap_ft": 8.0,
            },
        },
