"""Build the standalone Stage 22A disjoint residual-field notebook."""
...
OUTPUT = Path("notebooks/560_run_stage22a_residual_field.ipynb")
...
def build() -> None:
    cells = [
        markdown(
            ...
            "Stage 21のcandidate routerは終了します。この実験では候補を選ばず、A130 baseに対する"
            "A100/A160・selector・public OOFの行ごとの不一致とtarget-free軌跡特徴から非線形residual"
            "を学習します。Stage 21Aの63 wellsだけで学習し、Stage 21Bの58 wellsだけで固定評価します。"
            "well重複はゼロです。Kaggle提出は作りません。CPUランタイムを使用してください。\n"
        ),
        ...
        markdown(
            "## 固定された学習・検証split\n\nStage 21Aをtraining、Stage 21Bをvalidationとして再利用します。"
            "両者はwell単位で完全非重複です。outer suffix targetは学習ラベルまたは評価にだけ使い、"
            "特徴生成時のhidden-target invarianceも検査します。\n"
        ),
        code(
        ...
        markdown(
            "## Residual field\n\nStage 21よりPF計算は少なく、各cutでouter候補だけを生成します。"
            "学習特徴と検証をそれぞれ10 cutsごとに表示します。途中失敗時はこのrunだけを削除して"
            "最初から再実行します。\n"
        ),
        code(
            ...
            "    run_checked(['uv','run','rogii-prefix-residual-field','--config',"
            "'configs/experiment/stage22a_residual_field.yaml','--stage16b-run',str(stage16b_run),"
            ...
            "{key:summary[key] for key in ['stage22a_complete','promoted_to_stage22b','training_cuts',"
            "'training_wells','training_rows','validation_cuts','validation_wells',"
            "'training_validation_well_overlap','feature_count','base_rmse','candidate_rmse',"
            "'rmse_delta','well_p90_delta','bootstrap_95pct','gates','next_step']}\n"
        ),
        code(
            "import pandas as pd\n"
        ...
        markdown(
            "最後のsummary辞書とweight表を共有してください。primary weight 0.25の全gate通過時だけ"
            "Stage 22Bへ進みます。diagnostic weightを見てprimary判定を後付け変更しません。\n"
        ),
    ]
    notebook = {
        "cells": cells,
        "metadata": {
            ...
            "stage22a": {
                ...
                "training_validation_well_isolation": True,
                "model_family": "rowwise_hist_gradient_boosting",
            },
        },
        "nbformat": 4,
        "nbformat_minor": 5,
