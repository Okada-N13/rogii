"""Build the standalone Stage 20A Colab alignment benchmark."""
...
OUTPUT = Path("notebooks/520_run_stage20a_top_pf_alignment.ipynb")
...
def build() -> None:
    cells = [
        markdown(
            ...
            "Stage 19Cは代理base上のCVでは改善したものの、6.589 baseへ追加すると6.958へ悪化しました。"
            "このNotebookは、target-safe public OOF、A130 likelihood PF、SP45 projection、"
            "最終60/40 blendを固定200 cutsで再構成し、弱い3係数補正をcross-fitします。"
            "提出は作りません。CPUランタイムを使用してください。\n"
        ),
        ...
        markdown(
            "## 固定artifact\n\nStage 16B v003 manifestとStage 17Aのwell-isolated public OOFを使います。"
            "Stage 19CのLB結果やtest予測は教師・特徴へ使用しません。\n"
        ),
        code(
        ...
        markdown(
            "## 200-cut A130 replayとcross-fit\n\n5 folds × 5 prefix fractions × 8 cutsをSHA-256で固定抽出します。"
            "初回はNumba compileを含みます。途中で25 cutsごとに進捗が表示されます。\n"
        ),
        code(
            "RUN_ID='stage20a_top_pf_alignment_full_v001'; run_dir=artifact_dir/RUN_ID\n"
            "if not (run_dir/'summary.json').is_file():\n"
            "    run_checked(['uv','run','rogii-trajectory-base-alignment',"
            "'--config','configs/experiment/stage20a_top_pf_alignment.yaml',"
            ...
            "{\n"
            " 'stage20a_complete':summary['stage20a_complete'],"
            " 'promoted_to_stage20b':summary['promoted_to_stage20b'],"
            " 'sample_cuts':summary['sample_cuts'],'sample_wells':summary['sample_wells'],"
            " 'base_comparison':summary['base_comparison'],"
            " 'standard_base_rmse':summary['standard_report']['base_rmse'],"
            " 'standard_candidate_rmse':summary['standard_report']['candidate_rmse'],"
            " 'standard_delta':summary['standard_report']['rmse_delta'],"
            " 'bootstrap_95pct':summary['bootstrap_95pct'],'gates':summary['gates'],"
        ...
        markdown(
            "最後の辞書とweight表を共有してください。Stage 20Aで固定weight 0.10が全gateを通過した場合だけ"
            "全eligible cutsと短prefix proxyへ拡張します。ここから直接Kaggleへ提出しません。\n"
        ),
    ]
    notebook = {
        "cells": cells,
        "metadata": {
            ...
            "stage20a": {
                "submission": False, "standalone_setup": True, "sample_cuts": 200,
                "target_safe_public_oof": True, "a130_gr_sigma_multiplier": 1.3,
            },
        },
