"""Build the standalone Stage 25A temporal path decoder notebook."""
...
OUTPUT = Path("notebooks/620_run_stage25a_temporal_path_decoder.ipynb")
...
def build() -> None:
    cells = [
        markdown(
            ...
            "Stage 24Aの5 checkpointsを固定し、offsetを各row独立に使わず連続pathとして復号します。"
            "62 design-validation cutsだけを使うexploratory screenで、予約120 wellsは触りません。"
            "eligible profileがあっても提出や予約well確認へ直接進まず、500-cut OOF nested auditを"
            "先に行います。T4/L4で十分です。\n"
        ),
        ...
        code(
            ...
            "stage24a_run/'summary.json']+[stage24a_run/f'fold_{fold}.pt' for fold in range(5)]\n"
        ...
        code(
            ...
            "    command=[sys.executable,'-m','rogii.cli.emission_path_decoder','--config',"
            "'configs/experiment/stage25a_temporal_path_decoder.yaml','--stage16b-run',str(stage16b_run),"
            ...
            "{key:summary[key] for key in ['stage25a_complete','promoted_to_stage25b_oof_audit','device',"
            "'design_validation_cuts','design_validation_wells','selected_screen_profile','screen_role',"
            "'reserved_confirmation_used','next_step']}\n"
        ),
        code(
            "import pandas as pd\n"
        ...
        markdown(
            "summary辞書とprofile表を共有してください。この画面の最良profileをそのまま採用せず、"
            "eligibleの場合も500-cut OOF nested auditへ進みます。\n"
        ),
    ]
    notebook = {
        "cells": cells,
